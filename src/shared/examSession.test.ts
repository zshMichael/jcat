...
describe('examSession', () => {
  ...
  it('subtracts real elapsed time on tick, not 1000ms steps', () => {
    let { session } = reduce(idleSession(), { type: 'start', now: T0, projectRoot: '/p', baseline: {} })
  ...
  it('never goes negative and notifies zero only once', () => {
    const started = reduce(idleSession(), { type: 'start', now: T0, projectRoot: '/p', baseline: {} })
  ...
  it('round-trips JSON with schemaVersion', () => {
    const { session } = reduce(idleSession(), { type: 'start', now: T0, projectRoot: '/p', baseline: { x: ['Foo'] } })
