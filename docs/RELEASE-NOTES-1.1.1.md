...
- **API Key：** 写入系统安全存储（macOS Keychain 加密 / Windows 用户级加密）。从 v1.1.0 明文 `settings.json` 会自动迁移。渲染进程拿不到完整 Key。
...
- **安装包：**
  ...
  - `jcat-1.1.1-setup.exe` — Windows 默认包（x64）
...
- **API key:** Stored with OS secret storage (Keychain-backed encryption on macOS, user-level encryption on Windows). Plaintext keys from v1.1.0 migrate automatically. The renderer never receives the full key.
...
- **Installers:**
  ...
  - `jcat-1.1.1-x64-setup.exe` — Windows x64
  - `jcat-1.1.1-arm64-setup.exe` — Windows ARM64
  - `jcat-1.1.1-setup.exe` — default Windows package (x64)
...
- **API Key:** OS 보안 저장소(macOS Keychain 암호화 / Windows 사용자 수준 암호화). v1.1.0의 평문 Key는 자동 이전됩니다. 렌더러는 전체 Key를 받지 않습니다.
...
- **설치 파일:**
  ...
  - `jcat-1.1.1-x64-setup.exe` — Windows x64
  - `jcat-1.1.1-arm64-setup.exe` — Windows ARM64
  - `jcat-1.1.1-setup.exe` — Windows 기본 패키지(x64)
