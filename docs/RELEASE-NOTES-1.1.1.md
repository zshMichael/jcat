...
Builds are **not Apple-signed or notarized** and **not Windows-signed**. Verify downloads with `SHA256SUMS.txt` after this release is published. Do not disable OS security features to open Jcat.
...
- **AI Debug：** 可停止/取消，有超时、长度截断、SSE 流式解析和请求隔离，错误可翻译。产品方向仍是最小修复，不代写整道 FRQ。
...
- **macOS：** 当前未签名、未公证。第一次请在访达中右键 Jcat → 打开，不要关 Gatekeeper。
- **Windows：** 当前未代码签名，SmartScreen 可能显示“未知发布者”。x64 与自动识别包已在 CI 完成安装、PE 架构和启动存活检查。ARM64 包已完成构建与 payload 校验，尚未在原生 ARM64 设备上运行验证。x64 与 ARM64 请用对应安装包。
- **安装包：**
...
- **AI Debug:** Stop/cancel, timeouts, length truncation, SSE streaming, request isolation, translatable errors. Still a minimal fix, not a full FRQ solution.
...
- **macOS:** unsigned and not notarized. First open in Finder: right-click Jcat → Open. Do not turn off Gatekeeper.
- **Windows:** not code-signed; SmartScreen may show an unknown publisher. The x64 and auto-detect installers passed CI install, PE-arch, and launch-survive checks. The ARM64 installer is built and payload-checked; it has not been run on a native ARM64 device. Use the matching x64 or ARM64 installer.
- **Installers:**
  - `jcat-1.1.1.dmg` — macOS Apple Silicon
  - `jcat-1.1.1-x64-setup.exe` — Windows x64
  - `jcat-1.1.1-arm64-setup.exe` — Windows ARM64
...
- **AI Debug:** 중지/취소, 시간 제한, 길이 자르기, SSE 스트리밍, 요청 격리, 번역 가능한 오류. 최소 수정이며 FRQ 전체를 대신 쓰지 않습니다.
...
- **macOS:** 서명·공증되지 않았습니다. 처음에는 Finder에서 Jcat을 우클릭 → 열기. Gatekeeper를 끄지 마세요.
- **Windows:** 코드 서명이 없어 SmartScreen이 “알 수 없는 게시자”를 표시할 수 있습니다. x64와 자동 인식 설치 파일은 CI에서 설치·PE 아키텍처·실행 생존 검사를 통과했습니다. ARM64 패키지는 빌드와 payload 검증까지 끝났고, 실제 ARM64 기기에서 실행 검증은 아직 없습니다. x64 / ARM64 설치 파일을 구분하세요.
- **설치 파일:**
  - `jcat-1.1.1.dmg` — macOS Apple Silicon
  - `jcat-1.1.1-x64-setup.exe` — Windows x64
  - `jcat-1.1.1-arm64-setup.exe` — Windows ARM64
